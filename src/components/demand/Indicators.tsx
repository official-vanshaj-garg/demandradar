    ...
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
        ...
        <circle cx={size/2} cy={size/2} r={r} strokeWidth="4" fill="none"
                stroke="url(#meter-grad)" strokeLinecap="round"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center leading-none">
        ...
        <span className="font-mono text-[8px] uppercase tracking-widest text-muted-foreground">signal</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-muted">
        <div className="h-full rounded-full bg-gradient-to-r from-secondary to-primary" style={{ width: `${v}%` }} />
      </div>
    </div>
  );
}

const URGENCY_COLOR = ["", "oklch(0.78 0.10 220)", "oklch(0.78 0.13 190)", "oklch(0.80 0.17 70)", "oklch(0.74 0.20 35)", "oklch(0.68 0.22 25)"];
