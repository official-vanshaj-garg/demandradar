    ...
    <div className="rounded-2xl border border-border bg-glass p-5 glass">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-display text-sm font-semibold uppercase tracking-widest">Live signal feed</h3>
        </div>
        <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Bengaluru pilot</span>
      </div>
...
function SignalRow({ d, delay }: { d: DemandReport; delay: number }) {
  ...
  return (
    <li className="anim-ticker-in flex items-center gap-3 py-3" style={{ animationDelay: `${delay}s` }}>
      <span className="h-2 w-2 shrink-0 rounded-full anim-signal-blink" style={{ background: meta.color }} />
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          <span className="truncate text-sm">{d.title}</span>
          <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{ago}</span>
        </div>
