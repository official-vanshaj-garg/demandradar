...
export function RadarHero({ size = 480, pulses }: Props) {
  const items = pulses ?? BLR_ZONES.slice(0, 9).map((z, i) => ({
    x: z.x, y: z.y, intensity: 0.5 + ((i % 5) * 0.12),
  ...
  return (
    <div className="relative h-full w-full" style={{ aspectRatio: "1 / 1", maxWidth: size, maxHeight: size, marginInline: "auto" }}>
      <div className="absolute inset-0 rounded-full"
           style={{ background: "radial-gradient(circle at center, oklch(0.68 0.18 250 / 0.18), transparent 70%)" }} />
      <svg viewBox="0 0 100 100" className="relative h-full w-full">
        ...
        {/* base disc */}
        <circle cx="50" cy="50" r="48" fill="url(#rg-bg)" stroke="oklch(1 0 0 / 0.10)" strokeWidth="0.3" />
        {/* concentric rings */}
        {[12, 24, 36, 46].map((r) => (
          <circle key={r} cx="50" cy="50" r={r} fill="none" stroke="oklch(0.82 0.16 195 / 0.18)" strokeWidth="0.18" />
        ))}
        {/* crosshair */}
        ...
        {items.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="2.4" fill={p.color || "var(--primary)"}
                    style={{ filter: "drop-shadow(0 0 4px currentColor)" }} />
            <circle cx={p.x} cy={p.y} r="2.4" fill="none" stroke={p.color || "var(--primary)"} strokeWidth="0.4"
        ...
        <circle cx="50" cy="50" r="3.2" fill="none" stroke="oklch(0.82 0.16 195 / 0.6)" strokeWidth="0.3" />
      </svg>
      ...
      <div className="pointer-events-none absolute inset-0">
        {["NW","NE","SW","SE"].map((label, i) => (
          <span key={label}
                className={"absolute font-mono text-[9px] uppercase tracking-[0.2em] text-muted-foreground/70 " +
